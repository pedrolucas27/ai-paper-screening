...
Depois de calcular `Score Geral (0-10)` de **todos** os artigos, ordene em ordem decrescente. Critério de desempate, nesta ordem: (1) maior `C2_Reprodutibilidade` (reprodutibilidade é o critério mais diretamente verificável e, por isso, o desempate mais defensável entre os quatro); (2) maior `Score Combinado` do Estágio 2 (resume, só para fins de desempate, o desempenho combinado das duas etapas anteriores); (3) `Article Citation Count` (em última medida, como dado geral dos artigos em especifico). Atribua `Posição Ranking` (1 = melhor) sem pular números, mesmo em empate completo (atribua a mesma posição e prossiga a numeração normalmente a partir da próxima linha distinta).
